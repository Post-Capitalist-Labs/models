...
class CouncilAgent(Agent):
    def __init__(self, unique_id, model, initial_plan_min, initial_plan_max):
        ...
        # Attributes for equilibrium tracking
        self.time_to_equilibrium_balanced_proposals = None
        self.time_to_equilibrium_stability_matched_unmatched = None
        self.time_to_equilibrium_minimal_unmatched = None
        self.time_to_equilibrium_stability_adjustments = None
    ...
    def adjust_proposal(self, global_average, min_value, max_value):
        ...
        self.update_equilibrium_counters()
    ...
    def check_stability_in_adjustments(self):
        if len(self.proposal_history) < self.stability_window:
            return False

        avg_workers_proposals = sum(hist[1] for hist in self.proposal_history) / self.stability_window
        avg_consumers_proposals = sum(hist[2] for hist in self.proposal_history) / self.stability_window

        return abs(avg_workers_proposals - avg_consumers_proposals) <= 0.10 * avg_workers_proposals

    def update_equilibrium_counters(self):
        if self.check_balanced_proposals():
            if self.time_to_equilibrium_balanced_proposals is None:
                self.time_to_equilibrium_balanced_proposals = self.schedule.steps

        if self.check_stability_in_proposals():
            if self.time_to_equilibrium_stability_matched_unmatched is None:
                self.time_to_equilibrium_stability_matched_unmatched = self.schedule.steps

        if self.check_minimal_unmatched_proposals():
            if self.time_to_equilibrium_minimal_unmatched is None:
                self.time_to_equilibrium_minimal_unmatched = self.schedule.steps

        if self.check_stability_in_adjustments():
            if self.time_to_equilibrium_stability_adjustments is None:
                self.time_to_equilibrium_stability_adjustments = self.schedule.steps
...
class CouncilBasedEconomyModel(Model):
    ...
    def text_visualization(self):
        ...
        if self.time_to_equilibrium is not None:
            print(f"Time to Balanced Proposals Equilibrium: {self.time_to_equilibrium_balanced_proposals or 'Not yet reached'} steps")
            print(f"Time to Stability in Matched/Unmatched Equilibrium: {self.time_to_equilibrium_stability_matched_unmatched or 'Not yet reached'} steps")
            print(f"Time to Minimal Unmatched Proposals Equilibrium: {self.time_to_equilibrium_minimal_unmatched or 'Not yet reached'} steps")
            print(f"Time to Stability in Adjustments Equilibrium: {self.time_to_equilibrium_stability_adjustments or 'Not yet reached'} steps")
